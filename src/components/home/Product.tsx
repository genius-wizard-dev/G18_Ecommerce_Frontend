...
const ProductCard: FC<ProductCardProps> = ({ _id, name, price, thumbnailImage, discount }) => {
    return (
                ...
                <div className="flex items-center gap-2">
                    {discount ? (
                        <>
                            ...
                            <span className="text-gray-500 text-sm line-through">{price.toLocaleString()}₫</span>
                        </>
                    ) : (
        ...
        <section className="bg-white py-6">
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    {products.length > 0 &&
                        products.map((product: any) => <ProductCard key={product._id} {...product} />)}
                </div>
            </div>
        </section>
    );
