...
const CartPage = () => {
  ...
  const [currentCartItemData, setCurrentCartItemData] =
    useState<UpdateQuantityInput | null>(null);
  ...
  const handleApplyDiscount = (discountId: string) => {
    if (!discountId || !cartId || appliedProductIdList.length == 0 || !profile)
      return;
  ...
  const handleSelectAppliedProduct = (productId: string) => {
    if (appliedProductIdList.includes(productId))
      setAppliedProductIdList(
        appliedProductIdList.filter(
          (appliedProduct) => appliedProduct !== productId
        )
      );
  ...
  useEffect(() => {
    setAppliedProductList(
      cartItems.filter((cartItem: CartItem) =>
        appliedProductIdList.includes(cartItem.productId)
      )
    );
  ...
  useEffect(() => {
    const shopIdList: string[] = Array.from(
      new Set(cartItems.map((cartItem) => cartItem.shopId))
    );
  ...
  useEffect(() => {
    ...
    setTotalCartPrice(
      paymentMethod !== "PAYPAL"
        ? totalPrice
        : parseFloat((totalPrice / 26000).toFixed(2))
    );
  ...
  useEffect(() => {
    ...
    timerId = setInterval(() => {
      setTrigger(parseInt(localStorage.getItem("added") ?? "0"));
    }, 1000);
  ...
  useEffect(() => {
    if (trigger && profile) dispatch(getCart(profile.id));
  }, [trigger, profile]);
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleClearCart(profile.id)}
                >
                  Xóa tất cả
                </Button>
              </div>
                  ...
                  >
                    ...
                    <div className="flex-grow">
                      ...
                      <div
                        ...
                        onClick={() =>
                          handleSelectAppliedProduct(item.productId)
                        }
                      >
                        <input
                          type="checkbox"
                          className="relative top-[1px]"
                          checked={appliedProductIdList.includes(
                            item.productId
                          )}
                          readOnly={true}
                        />
