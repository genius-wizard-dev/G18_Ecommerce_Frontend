...
import { resetProductForm, setIsUploading, setSelectedProduct } from "@/redux/slices/shopManagerSlice";
import {
    createProduct,
    deleteProduct,
    fetchProducts,
...
const ProductManagement: React.FC = () => {
    ...
    const { products, selectedProduct, isLoading, isUploading, isDeleting, inventory, dataFetched } = useAppSelector(
        (state) => state.shopManager
    );
    ...
    useEffect(() => {
        const fetchAllInventories = async () => {
            try {
                ...
                const productsNeedInventory = products.filter((product) => product._id && !inventory[product._id]);
    ...
    const handleDelete = async (id: string) => {
        try {
            if (profile?.shopId) await dispatch(deleteProduct({ id, shopId: profile?.shopId }));
    ...
    const handleToggleActive = async (id: string, isActive: boolean) => {
        try {
            ...
            toast.error("Không thể cập nhật trạng thái sản phẩm. Vui lòng thử lại sau.");
        }
    };

    return (
        <section id="products" className="space-y-6 relative">
            <h2 className="text-3xl font-bold tracking-tight flex items-center">Quản lý sản phẩm</h2>

            {/* Product Stats */}
            <ProductStatistics totalProducts={statistics.totalProducts} isLoading={statistics.isLoading} />
