...
import { CLOUDINARY_CONFIG, uploadMultipleToCloudinary, uploadToCloudinary } from "@/utils/cloudinary.config";
import { getImageUrl } from "@/utils/getImage";
import FileUpload from "@/utils/upload.images.tsx";
import { CheckCircle, Image as ImageIcon, Package, Pencil, Plus, Tag, Trash2 } from "lucide-react";
...
const ProductManagement: React.FC = () => {
    ...
    const handleNestedObjectChange = (parent: string, key: string, value: string) => {
        const values = value.split(",").map((item) => item.trim());
        setAttributes((prev) => {
    ...
    const attributeFields = useMemo(() => {
        ...
        try {
            ...
            const categoryLabel = PRODUCT_CATEGORIES.find((cat) => cat.value === category)?.label;

            return (
                <div className="mt-4 border-t pt-4">
                    <h3 className="font-medium mb-3">Thuộc tính sản phẩm {categoryLabel}</h3>
                            ...
                            if (fieldType === "ZodObject") {
                                ...
                                return (
                                    <div key={key} className="space-y-2 md:col-span-2">
                                        ...
                                        {Object.entries(nestedShape).map(([nestedKey, nestedFieldSchema]) => {
                                            const nestedFieldType = (nestedFieldSchema as any)._def.typeName;
                                            if (nestedFieldType === "ZodArray") {
                                                ...
                                                return (
                                                    <div key={`${key}-${nestedKey}`} className="ml-4 space-y-2">
                                                        ...
                                                        <Input
                                                            ...
                                                            value={displayValue}
                                                            onChange={(e) =>
                                                                handleNestedObjectChange(key, nestedKey, e.target.value)
                                                            }
                            ...
                            if (fieldType === "ZodArray") {
                                ...
                                return (
                                    <div key={key} className="space-y-2">
                                        <Label htmlFor={key}>
                                            {key.charAt(0).toUpperCase() + key.slice(1)} (ngăn cách bằng dấu phẩy)
                                        </Label>
                                        <Input
                                            id={key}
                                            value={displayValue}
                                            onChange={(e) => handleArrayAttributeChange(key, e.target.value)}
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (thumbnailFile) {
                thumbnailPublicId = await uploadToCloudinary(thumbnailFile, CLOUDINARY_CONFIG);
            }

            // Upload multiple images to Cloudinary
            let imagePublicIds: string[] = [];
            if (imageFiles.length > 0) {
                imagePublicIds = await uploadMultipleToCloudinary(imageFiles, CLOUDINARY_CONFIG);
            }
            ...
            if (editId !== null) {
                ...
                const categoryEnum = category as keyof typeof ProductCategory;
                const attributeSchema = getAttributeSchemaForCategory(ProductCategory[categoryEnum]);
                ...
                if (profile?.shopId) {
                    try {
                        const response = await api.post<ProductResponse>(ENDPOINTS.PRODUCT.CREATE, newProduct);
        ...
        try {
            ...
            const categoryString =
                Object.keys(ProductCategory).find(
                    (key) => ProductCategory[key as keyof typeof ProductCategory] === product.category
                ) || "";
            ...
            if (product.attribute) {
                ...
                setTimeout(() => {
                    for (const key in product.attribute) {
                        const attributeValue = product.attribute[key as keyof typeof product.attribute];
                        ...
                        if (
                            ...
                            for (const nestedKey in attributeValue as Record<string, unknown>) {
                                const nestedValue = (attributeValue as Record<string, unknown>)[nestedKey];
                                if (Array.isArray(nestedValue)) {
                                    ...
                                    handleNestedObjectChange(key, nestedKey, nestedValue.join(", "));
                                }
                            }
                        }
    ...
    return (
        <section id="products" className="space-y-6">
          ...
          <CardContent>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{editId !== null ? "Sửa sản phẩm" : "Thêm sản phẩm"}</CardTitle>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <Button
                                ...
                                disabled={isUploading || !name || !price || !quantity || !category || !thumbnailFile}
                            >
                                {editId !== null ? (
                                    <>
